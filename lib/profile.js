...
// A batch of words from a curriculum/exam list she has not necessarily
// encountered yet -- source 'band' (declared in WORD_SOURCES since phase 1,
// never before written by any live feature). Deliberately the OPPOSITE of
// markWordKnown: status 'learning' (she still has to prove it, same as a
// story tap), and an EXISTING entry is left completely untouched -- never
// downgraded from 'known'/'candidate' back to 'learning', never even given a
// missing `he`, so re-running this on a page she revisits costs nothing and
// can never take back ground a real tap or quiz already earned.
export function collectBandWords(profile, { words, now = new Date().toISOString() }) {
  if (!Array.isArray(words)) return profile;
  for (const w of words) {
    if (!isPlainObject(w)) continue;
    const k = String(w.lemma).trim().toLowerCase();
    if (k === '') continue;
    if (profile.words[k]) continue;
    profile.words[k] = {
      status: 'learning',
      source: 'band',
      he: typeof w.he === 'string' && w.he.trim() !== '' ? w.he : null,
      taps: 0,
      firstSeen: now,
      lastSeen: now,
    };
