...
export function buildAllowedSet(profile, band1, band2 = null) {
...
- You may use AT MOST 3 words that are not on the ALLOWED WORD LIST. Every word not on the list MUST have an entry in the glossary.
...
export async function generateChapter({ profile, band1, band2 = null, chat, now = new Date().toISOString(), maxAttempts = 3 }) {
  const allowedSet = buildAllowedSet(profile, band1, band2);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (v.errors.length > 0 && v.errors.every((e) => e.startsWith('glossary:'))) {
      if (!Array.isArray(candidate.glossary)) {
        candidate.glossary = [];
      }
      const glossaryWords = new Set(
        candidate.glossary
      ...
      const missing = v.unknown.filter((token) => {
        const covered = glossaryWords.has(token) || baseForms(token).some((b) => glossaryWords.has(b));
        return !covered;
      });
      ...
      for (const w of missing) {
        try {
          ...
          if (t && typeof t.he === 'string' && t.he.trim() !== '') {
            candidate.glossary.push({ word: w, he: t.he });
          }
        } catch (err) {
          // skip this word; verification will surface any remaining gap
        }
      }

      const v2 = verifyChapter(candidate, allowedSet);
      if (v2.ok) {
        candidate.coverageRatio = v2.ratio;
        return {
          ok: true,
          chapter: candidate,
          summarySoFar: (typeof raw.summarySoFar === 'string' && raw.summarySoFar) || previousSummary || '',
      ...
      forbidden = v2.unknown;
      lastErrors = v2.errors;
      continue;
    }
