...
grep -q "magic-vet-v25" public/sw.js || fail "sw.js CACHE should be magic-vet-v25 after step 2.6"

# 9. boundary: exactly two paths changed
CHANGED=$(git status --porcelain -- public/ tests/ scripts/ | awk '{print $2}' | LC_ALL=C sort | tr '\n' ' ')
EXPECTED="public/quiz.js tests/quiz-kinds.test.js "
...
if [ "$RC" = "0" ]; then echo "STEP-2.7-OK"; else echo "STEP-2.7-FAILED"; fi
exit $RC
