...
| **B** | the chapter-first quiz has almost nothing of hers to ask. **CORRECTED 2026-08-02 (phase 3): this row said "only 1 of her ~40 words has a quiz item" and that garbled the measurement.** The real figure was about her ~36 GLOSSARY words, of which exactly one intersected the bank. Her PROFILE words were separately measured as fully covered. Glossary != profile words, and the difference IS the phase: the words she taps in her story are `learning`, which the top-up lister structurally cannot see. | measured during word-polish | coverage |
...
| **E** | `/c/Users/dkreinov/english-app-sandbox/profile.json` holds **her real vocabulary** (12 words -- REDACTED 2026-08-02, phase 3: the words themselves were named here in plain text. D27 deleted every profile FILE but never covered the .oplan record, so her vocabulary sat in the repository for two days. Counts only from now on.) | found 2026-08-01 during the D27 deletion | data hygiene |
...
`{lemma, sense, sentence-with-`___`, answer, distractors}` -- **CORRECTED 2026-08-02: it is EXACTLY 8 distractors** (`lib/quiz-item.js:24` `DISTRACTOR_COUNT`, hard error at `:246`). `>= 5` is `isUsableItem`'s RUNTIME tolerance, not the authoring contract); `isUsableItem` in the frozen
...
~~Generation follows the existing house style (`scripts/build-item-bank.js` uses `gpt-4.1-mini` via the OpenAI chat API). **PAID API**~~ **WRONG ON BOTH COUNTS, CORRECTED 2026-08-02 (ruling R-F3-1).** `scripts/build-item-bank.js` builds the **placement** bank, writes `data/placement-items.json`, and has never written a byte into `public/quiz/`. The real precedent is owner-level decision **D7** (`.oplan/word-quiz/design.md:36`): *"Generated by Claude via oplan workers, cheap tier -- **no paid API**, no runtime LLM, no second key"*. Phase 3 spent **zero money**. **The owner must still approve the batch and see samples before any item reaches her** -- that half stands and was honoured. No change to `public/quiz.js` or `public/quiz-core.js`.
